...
export default async function RefundsDashboardPage() {
    ...
    const [{ totalRefunds = 0 } = {}] = await database
        .select({
            totalRefunds: sql<number>`SUM(CASE WHEN ${PurchaseTable.refundedAt} IS NOT NULL THEN ${PurchaseTable.pricePaidInCents} ELSE 0 END) / 100`,
        })
        .from(PurchaseTable);
    ...
    const [{ totalSales = 0 } = {}] = await database
        .select({
            totalSales: sql<number>`SUM(${PurchaseTable.pricePaidInCents}) / 100`,
        })
        .from(PurchaseTable);

    return (
        <RefundSalesDashboard
